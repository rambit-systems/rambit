...
use crate::resources::cache::cache_name_is_available_query_scope;

#[component]
pub(super) fn CacheNameValidation() -> impl IntoView {
  ...
  let is_available_resource = expect_context::<QueryClient>().resource(
    cache_name_is_available_query_scope(),
  ...
  let availability_hint_suspend = move || {
    Suspend::new(async move {
      match is_available_resource.await {
        ...
        Err(_) => view! {
          <p class="text-critical-11 text-sm">
            "Unable to check cache name availability"
          </p>
        }
        .into_any(),
      }
